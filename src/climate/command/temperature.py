...
from climate import adapter, dataframe, initialiser, model, plot, presenter
from climate.command import commanda, helpers


@commanda.command(graph_name="climateGraph")
def add_temperature(
    locale: str | model.locale.Locale, minimum, maximum, date=None
) -> monad.EitherMonad[Dict]:
    g = helpers.climate_graph()
    result = model.temperature.record(
        g=g, locale=locale, minimum=minimum, maximum=maximum, for_date=date
    )
    return result
...
def plot_temperatures(channel: str):
    g = helpers.climate_graph()
    result = plot.temperature_plot.locale_temperatures(
        df=dataframe.temperature.locale_temperatures(g)
    )
    if result.is_left():
        return result
    presenter.plot_to_channel(
        result.value,
