...
from clojos_common.util import singleton

from climate import rdf, repo

DB_MAP = {
    'climate_graph': (Path(__file__).parent.parent.parent.parent / "data" / "db" / "climate.ttl")
}
...
class Db:
    ...
    def graph_for(self, name: str) -> Graph:
        if name not in self.graphs.keys():
            return None
        return repo.GraphRepo(self.get_graph_attr(name), name)
    ...
    def get_graph_attr(self, name):
        return getattr(self, self.graph_attr_name(name), None)
