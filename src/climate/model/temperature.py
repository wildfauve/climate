...
def _from_dto(record):
    locale = model.locale._to_locale_result(
        name=record.locale_name, subject=record.locale_subject
    )
    return MinMaxTemperatureRecord(
...
def _to_model(
    ...
    return MinMaxTemperatureRecord(
        subject=_record_sub(locale.value, record_date),
        minimum=minimum,
        maximum=maximum,
        locale=locale.value,
...
def _fill_blanks(g, records):
    ...
    grouped = [
        (url, list(rec))
        for url, rec in groupby(records, lambda rec: rec.locale.subject)
    ]
